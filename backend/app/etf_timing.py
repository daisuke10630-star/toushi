...
理論上は間に合う。日次バッチは寄り付き前の6:10 JSTを狙って設定している
（.github/workflows/daily-report.yml）。

ただし実際に運用したところ、GitHub Actionsのスケジュール実行（cron）は
無料枠・低頻度リポジトリだと数時間〜半日近く遅れることが分かった
（2026-08-04〜08-10の実測で、6:10 JST狙いが実際には7:03〜15:17 JSTの間で
バラバラに実行され、寄り付きに間に合わなかった日が複数あった）。
このため**自動バッチだけには頼れない**。実際に売買判断に使うときは、
寄り付き前に必ず「最新データに更新」ボタンを押して確定させること。
自動バッチは失敗時の保険程度の位置づけ。
"""
